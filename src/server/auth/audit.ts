 ...
 * Log an audit event.
 *
 * Metadata automatically dibersihkan dari data sensitif.
 */
export async function logAuditEvent(
  ...
  // Salin metadata supaya object asli tidak ikut berubah.
  ...
  // Simpan juga ke memory sebagai fallback.
  memoryAuditLogs.unshift(logEntry);

  // Batasi maksimal 500 log di memory.
  if (memoryAuditLogs.length > 500) {
    memoryAuditLogs.pop();
  }

  // Kalau Supabase belum dikonfigurasi,
  // cukup gunakan memory log.
...
export async function getAuditLogs(
  ...
  // Fallback kalau Supabase belum dikonfigurasi.
  if (!isSupabaseConfigured()) {
  ...
  try {
    ...
    if (error) {
      ...
      // tetap kembalikan memory logs.
      return memoryAuditLogs.slice(
        0,
        limit,
      );
    }
