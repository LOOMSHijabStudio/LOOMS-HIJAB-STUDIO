...
export default function ShopAdminPage() {
  ...
  async function loadProducts() {
    try {
      ...
      const response = await fetch(
        "/api/admin/placements?placement=SHOP",
        {
          cache: "no-store",
        }
      );
  ...
  async function removeFromShop(productId: string) {
    const confirmed = window.confirm(
      "Hapus produk ini dari Shop?\n\nProduk tidak akan dihapus dari database."
    );
    ...
    try {
      ...
      if (!data.success) {
        alert(data.error || "Gagal menghapus produk dari Shop");
        return;
      }
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Kelola produk yang ditampilkan di halaman Shop.
          </p>
        </div>
