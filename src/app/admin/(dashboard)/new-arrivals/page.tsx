...
export default function NewArrivalsAdminPage() {
  ...
  async function removeFromNewArrivals(productId: string) {
    const confirmed = confirm(
      "Hapus produk ini dari New Arrivals?\n\nProduk tidak akan dihapus dari database."
    );
    ...
    try {
      ...
      if (!data.success) {
        alert(data.error || "Gagal menghapus produk dari New Arrivals");
        return;
      }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Kelola produk yang ditampilkan di bagian New Arrivals.
          </p>
        </div>
                  ...
                  {filteredProducts.map((product) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() =>
                            removeFromNewArrivals(product.id)
                          }
                          className="rounded-lg bg-red-50 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-100"
                        >
                          Hapus dari New Arrivals
                        </button>
                      </td>
                    </tr>
                  ))}
